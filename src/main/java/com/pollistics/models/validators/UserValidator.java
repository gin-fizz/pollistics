...
public class UserValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if(!user.getPassword().matches("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\\S+$).{8,}$")) {
			errors.rejectValue("password", "invalid.password", "Password has to be at least 8 characters long and needs to contain at least one uppercase letter, one lowercase letter and one number");
		}
	}
}
